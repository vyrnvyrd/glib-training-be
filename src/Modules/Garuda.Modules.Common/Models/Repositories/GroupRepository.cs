// <copyright file="GroupRepository.cs" company="CV Garuda Infinity Kreasindo">
...
namespace Garuda.Modules.Common.Models.Repositories
{
    ...
    {
        public async Task AddOrUpdate(Group model)
        {
        ...
        public async Task<Group> GetData(Guid id)
        {
            var data = await this.dbSet.FirstOrDefaultAsync(x => x.Id == id);
            return data;
        }

        public async Task<List<Group>> GetData(bool isActive)
        {
            if (isActive)
            {
