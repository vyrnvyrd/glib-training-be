// <copyright file="UnitRepository.cs" company="CV Garuda Infinity Kreasindo">
...
namespace Garuda.Modules.Common.Models.Repositories
{
    ...
    {
        public async Task AddOrUpdate(Unit model)
        {
        ...
        public async Task<Unit> GetData(Guid id)
        {
            var data = await this.dbSet.FirstOrDefaultAsync(x => x.Id == id);
            return data;
        }

        public async Task<List<Unit>> GetData(bool isActive)
        {
            if (isActive)
            {
